...
- **A flat capture option costs five edits here.** Anything on `CaptureOptions` that the React path must be able to set has to be threaded through `UtmConfig`, `ResolvedUtmConfig` (in `@/src/types`), `DEFAULT_CONFIG`, `createConfig()`, and `mergeConfig()`, then forwarded in `@/src/react/useUtmTracking.ts`. `lowercaseValues` is the current example. Missing any one of these silently drops the option on the React path only.
